...
#if defined(NDEBUG)
# error "coochie cannot be compiled without assertions."
#endif
...
CBigNum bnProofOfStakeLimit(~uint256(0) >> 32);
...
const string strMessageMagic = "coochie Signed Message:\n";
...
void RegisterWallet(CWalletInterface* pwalletIn) {
    g_main_signals.SyncTransaction.connect(boost::bind(&CWalletInterface::SyncTransaction, pwalletIn, _1, _2, _3));
    g_main_signals.EraseTransaction.connect(boost::bind(&CWalletInterface::EraseFromWallet, pwalletIn, _1));
    g_main_signals.UpdatedTransaction.connect(boost::bind(&CWalletInterface::UpdatedTransaction, pwalletIn, _1));
    g_main_signals.SetBestChain.connect(boost::bind(&CWalletInterface::SetBestChain, pwalletIn, _1));
    g_main_signals.Inventory.connect(boost::bind(&CWalletInterface::Inventory, pwalletIn, _1));
    g_main_signals.Broadcast.connect(boost::bind(&CWalletInterface::ResendWalletTransactions, pwalletIn, _1));
}

void UnregisterWallet(CWalletInterface* pwalletIn) {
    g_main_signals.Broadcast.disconnect(boost::bind(&CWalletInterface::ResendWalletTransactions, pwalletIn, _1));
    g_main_signals.Inventory.disconnect(boost::bind(&CWalletInterface::Inventory, pwalletIn, _1));
    g_main_signals.SetBestChain.disconnect(boost::bind(&CWalletInterface::SetBestChain, pwalletIn, _1));
    g_main_signals.UpdatedTransaction.disconnect(boost::bind(&CWalletInterface::UpdatedTransaction, pwalletIn, _1));
    g_main_signals.EraseTransaction.disconnect(boost::bind(&CWalletInterface::EraseFromWallet, pwalletIn, _1));
    g_main_signals.SyncTransaction.disconnect(boost::bind(&CWalletInterface::SyncTransaction, pwalletIn, _1, _2, _3));
}

void UnregisterAllWallets() {
    g_main_signals.Broadcast.disconnect_all_slots();
    g_main_signals.Inventory.disconnect_all_slots();
    g_main_signals.SetBestChain.disconnect_all_slots();
    g_main_signals.UpdatedTransaction.disconnect_all_slots();
    g_main_signals.EraseTransaction.disconnect_all_slots();
    g_main_signals.SyncTransaction.disconnect_all_slots();
}

void SyncWithWallets(const CTransaction &tx, const CBlock *pblock, bool fConnect) {
    g_main_signals.SyncTransaction(tx, pblock, fConnect);
}

void ResendWalletTransactions(bool fForce) {
    g_main_signals.Broadcast(fForce);
}
...
int nSubsidyHalvingInterval = 840960; //  ((60 * 24 * 365) / 2.5) * 4  = ((minutes year) / min per block) * years to half)
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 1900000 * COIN; /// for the swap on nova, then 100% of remainder will be distributed by Faucets, Bounties and Air Drops. Zero pre-mined coins will stay with myself.
...
{
    // nCoinAge is coins/day
    // so nSubsidy should be coins/days * reward for per 1 day
...
//alpha is 2/(N+1) where N is the number of past blocks we want to consider
//this results in alpha = 2/(100 + 1) =  0.01980198... since it is a repeating decimal we will
//cut it off 8 places after the decimal for no reason other than 1 satoshi is 8 places to the right of the decimal
double alpha = 0.01980198;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if(pindexLast->nHeight < 6)
    {
        return bnTargetLimit.GetCompact(); // first 5 blocks
    }
    int maxDepth = pindexLast->nHeight - 3;
    if(maxDepth > 100)
    {
        maxDepth = 100;
    }
    CBlockIndex* pindexCurrent = pindexLast;
    double ewma = 0.0;
    double one = 1.0;
    while(maxDepth > 0)
    {
        pindexCurrent = pindexCurrent->pprev;
        maxDepth = maxDepth - 1;
    }
    ewma = (double)(pindexCurrent->GetBlockTime() - pindexCurrent->pprev->GetBlockTime());
    while(pindexCurrent != pindexLast)
    {
        int64_t previousSpacing = pindexCurrent->GetBlockTime() - pindexCurrent->pprev->GetBlockTime();
        ewma = sqrt( ((one - alpha) * pow(ewma, 2.0)) + (alpha * pow(previousSpacing, 2.0)) );
        pindexCurrent = pindexCurrent->pnext;
    }

    LogPrintf("ewma = %f and expected = %i\n", ewma, TARGET_SPACING);
    CBigNum bnNew;
    bnNew.SetCompact(pindexPrev->nBits);
    /// cap it at a 20% increase in diff and a 40% decrease in diff
    if(ewma < 130)
    {
        ewma = 130;
    }
    ...
    int64_t newma = (int64_t)ewma;
    LogPrintf("ewma used = %i \n", newma);
    bnNew *= newma;
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    if ((pindexNew->nHeight % 20160) == 0 || (!fIsInitialDownload && (pindexNew->nHeight % 144) == 0))
    {
        ...
        g_main_signals.SetBestChain(locator);
    }
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    pindexNew->SetStakeModifier(fGeneratedStakeModifier);
    pindexNew->nStakeModifier = ComputeStakeModifier(pindexNew->pprev, IsProofOfWork() ? hash : vtx[1].vin[0].prevout.hash);
    ...
    if (pindexNew == pindexBest)
    {
        ...
        g_main_signals.UpdatedTransaction(hashPrevBestCoinBase);
...
bool IsCanonicalBlockSignature(CBlock* pblock, bool checkLowS)
{
